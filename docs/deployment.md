...
Keep Vercel on its default Python install/bundling behavior. A custom `installCommand` that installed `requirements-api.txt` caused Vercel to disable dependency optimization and fail with a 687.94 MB bundle, above Vercel's 245 MB function limit. `requirements-api.txt` remains the API runtime dependency reference for local/API-only installs and Cloud Run-style review, but `vercel.json` intentionally does not set a custom install command. Streamlit, Plotly, and SHAP remain in the root `requirements.txt` for Streamlit Cloud; Vercel's default Python bundler prunes unused imports for the Flask entrypoint. If the latest registered Hopsworks model is a TensorFlow artifact, or if Vercel still hits Hopsworks cold-start, dependency, memory, or timeout limits, use Cloud Run for the Flask API because the included Dockerfile installs `requirements-ml.txt` and is sized for heavier ML serving.
Both `requirements.txt` and `requirements-api.txt` pin `scikit-learn==1.8.0` because the current Hopsworks registry version 9 artifact was serialized with scikit-learn 1.8.0.

`/ready` fails on missing non-defaultable runtime secrets such as OpenWeather, Hopsworks, or MongoDB credentials. It also returns `recommended_env` booleans for explicit city/timezone/model-pin variables. Missing recommended variables should still be fixed in Vercel settings, but valid application defaults keep the service operational.
